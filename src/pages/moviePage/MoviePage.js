...
const MoviePage = () => {
  ...
  const isMovie = movieData.isTvShow ? 1 : 0;
  ...
  useEffect(() => {
    fetch(
      `https://api.adjaranet.com/api/v1/movies/${movieData.id}/season-files/${isMovie}?source=adjaranet`
    )
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => console.log(err));
  }, [movieData.id, isMovie]);

  const trailerLanguageHandler = value => {
    setTrailerUrl(movieData.trailers.data[value].fileUrl);
  };

  const dubsLanguageHandler = value => {
    setMovieUrl(moviePlayerData.files[value].files[1].src);
  };
            ...
            <div className={classes.moviePlayerWrapper}>
              ...
              {movieData.isTvShow && (
                <SeriesPlayer
                  url={movieUrl}
                  playerData={{
                    seasons: movieData?.lastSeries.data.season,
              ...
              <div
                ...
                className={
                  activeBtn === 'მსახიობები'
                    ? classes.activeSimilarFilmsTitle
                    : ''
                }
              >
                მსახიობები
              ...
              <div>
                <div className={classes.mobileInfoWrapper}>
                  ...
                  <div className={classes.mobileTextContainer}>
                    ...
                    <p>
                      IMDB:{' '}
                      <span>{movieData.rating.imdb.score.toFixed(1)}</span>
                    </p>
                  </div>
                </div>
